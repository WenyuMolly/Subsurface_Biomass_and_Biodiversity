...
####################################
gridCells = read.csv("metadata_with_merged_depth_and_gradient.csv",stringsAsFactors = FALSE)
...
cv.rsq = vector(length = bootstraps)
...
for (n in 1:nrow(myIndices)) {
  print(paste("Bootstrap iteration:", n))
  ####################################
  ...
  ## Wenyu: modified this equation to use the gradient from the XGboost model
...
####################################
 ...
 ################################
    ...
    results = foreach(i = 1:length(depthsToIterate), .combine = "c") %dopar% {
      ...
      if(patch$FID[1]%in%c(GreenlandFID,AntarcFID)){patch$temperature = 0 + mySlices * gridCells[i, ]$gradient / 1000}
      else{
...
write.table(cv.biomass,file = "glm_depthtemp122_cv.biomass.csv",sep=",",row.names=FALSE,col.names=FALSE)
write.table(cv.error,file = "glm_depthtemp122_cv.error.csv",sep=",",row.names=FALSE,col.names=FALSE)
write.table(cv.byGridResult, file = 'glm_depthtemp122_cvGridResult.csv',sep=',',row.names=FALSE,col.names=FALSE)
write.table(cv.rsq, file = "glm_depthtemp122_cv.rsq.csv", sep = ",", row.names = FALSE, col.names = FALSE)
